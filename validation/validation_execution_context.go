...
func validateCountable(count int, constraints []CountableConstraint) ValidateFunc {
	return func(ctx context.Context, validator *Validator) (*ViolationListError, error) {
		...
		for i := range constraints {
			err := violations.AppendFromError(
				constraints[i].ValidateCountable(ctx, validator, count),
			)
			if err != nil {
				return nil, err
			}
	...
	return func(ctx context.Context, validator *Validator) (*ViolationListError, error) {
		...
		for i := range values {
			for _, constraint := range constraints {
				err := violations.AppendFromError(
					constraint.ValidateString(ctx, validator.AtIndex(i), &values[i]),
				)
				if err != nil {
					return nil, err
				}
...
func validateEach[T any](
	values []T,
	validateFunc func(ctx context.Context, validator *Validator, value *T) error,
) ValidateFunc {
	return func(ctx context.Context, validator *Validator) (*ViolationListError, error) {
		violations := NewViolationList()

		for i := range values {
			if err := validateFunc(ctx, validator.AtIndex(i), &values[i]); err != nil {
				if vErr := violations.AppendFromError(err); vErr != nil {
					return nil, vErr
				}
			}
		}
...
func validateEachNumber[T Numeric](values []T, constraints []NumberConstraint[T]) ValidateFunc {
	return validateEach(values, func(ctx context.Context, validator *Validator, value *T) error {
		for _, constraint := range constraints {
			if err := constraint.ValidateNumber(ctx, validator, value); err != nil {
				return err
			}
		}
...
func validateEachComparable[T comparable](
	values []T,
	constraints []ComparableConstraint[T],
) ValidateFunc {
	return validateEach(values, func(ctx context.Context, validator *Validator, value *T) error {
		for _, constraint := range constraints {
			if err := constraint.ValidateComparable(ctx, validator, value); err != nil {
				return err
			}
		}
...
func validateComparable[T comparable](
	value *T,
	constraints []ComparableConstraint[T],
) ValidateFunc {
	return func(ctx context.Context, validator *Validator) (*ViolationListError, error) {
		...
		for i := range constraints {
			err := violations.AppendFromError(constraints[i].ValidateComparable(ctx,
				validator,
				value),
			)
			if err != nil {
...
func validateComparables[T comparable](
	values []T,
	...
	return func(ctx context.Context, validator *Validator) (*ViolationListError, error) {
		...
		for i := range constraints {
			err := violations.AppendFromError(constraints[i].ValidateComparables(ctx,
				validator,
				values,
			))
			if err != nil {
